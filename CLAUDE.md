...
- **Shop-global `value_id` resolution (do not regress):** the `Packing` sales attribute and its values (`1PCS`, `20PCS`, `Bubble Wrap`, …) are **shared shop-wide** (the same `value_id` appears on every product). Edit Product **silently drops** a value sent by name **without** its `value_id` while still returning success — so a pack size that exists elsewhere but not on this product would never get created. The runner resolves each requested pack size's id: from this product's own variant if present, else from a **donor product** in the catalog that already uses it (`_resolve_global_value_ids` → `fetch_product_detail_raw`), passed to `build_edit_payload(extra_value_ids=…)`. Only a value name used by **no** product is sent without an id (TikTok creates it fresh). **After the write the runner re-reads the product (`_verify_variants_created`) and reports `⚠️ sebagian gagal dibuat` + exit 1 if any requested value is missing** — never a false "berhasil".
