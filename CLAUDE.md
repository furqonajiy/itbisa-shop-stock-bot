...
## Tiered pricing (`/harga_set`) — `src/harga_set_price.py` (TikTok Shop only for now)
Set tiered ("Harga Grosir"-style) prices for one **exact** base SKU. Input is `(JUMLAH HARGA)` pairs = a unit price per quantity band, e.g. `1 749 50 739 100 699` → 1–49=Rp749, 50–99=Rp739, 100+=Rp699. `parse_tiers` validates + sorts; `unit_price_for_quantity(tiers, qty)` bands by the largest `start_qty ≤ qty` (below the lowest start → `None`). **TikTok Shop:** each pack-size variant is priced by the tier its multiplier `M` bands into, listing price = `unit_price × M` (1PCS→749, 50PCS→739×50, 1000PCS→699×1000) via `tiktokshop_client.update_price_batch`; variants below the lowest tier are skipped + reported (best-effort). Runs through `scripts/harga_set.py` (`--sku`, `--tiers q1 p1 …`, `--dry-run`) on `harga.yml`. **Shopee "Harga Grosir" is NOT wired yet** (deliberately deferred — the summary says so); when added it is one listing: base price = first tier, wholesale tiers for the rest. Pure tier logic is unit-tested (`tests/test_harga_set.py`); the API write is exercised via `--dry-run`.
...
- `/harga_set`: `send_harga_set_summary` (per-variant TikTok Shop price `Rp{listing} (Rp{unit}/pcs)`, skipped-variant note, Shopee "belum diaktifkan" line).
